...
return init();
  ...
  function quit(cb) {
    ...
    var err = new Error('This FS is closed');
    client  = _quit(client);
    cqlg    = _quit(cqlg);
    ...
    var ret = {
      ...
      _read_block: function(page, cb) {
        client.exa(cqlg.read_block(bid, page), function(err, inf) {
          ...
          if (inf.rowLength != 1) {
            return cb(null, new Buffer(blocksz), 0);
          }
          var blk = inf.rows[0];
          cb(null, blk.block, blk.usesz);
        });
      },
      ...
      _read_buffer: function(fpos, flen, cb) {
        ...
        function next_blk() {
          if (page < endblk) {
            ret._read_block(page, function(err, rbuf, usesz) {
              if (err) return cb(err);

              if (page == begblk) {
                rbuf.copy(buffer, 0, sp, usesz);
                readlen += usesz - sp;
              } else {
      ...
      _write_buffer: function(fpos, buffer, cb) {
        ...
        function read_blk(beg, next) {
          ret._read_block(page, function(err, rbuf, _usesz) {
            if (err) return cb(err);
            rbuf.copy(wbuf, beg, beg);
            usesz = Math.max(usesz, _usesz);
            next();
          });
        }
  ...
  function rename(oldPath, newPath, callback) {
    ...
    pathToNodeid('rename', newPath, PTNI_FL_NOT_LAST, function(err, targetdir) {
      ...
      pathToNodeid('rename', oldPath, null, function(err, inf) {
        if (err) return callback(err);
        var _c = cqlg.move_path(inf.path.id, inf.path.parent,
  ...
  function fchown(fd, uid, gid, callback) {
    ...
    fd_pool.getfd(fd, function(err, inf) {
      if (err) return callback(err);
      var c = cqlg.change_owner(inf.node.id, uid, gid);
      client.exax(c, make_cass_cb(callback, path, 'fchown'));
    });
  }
  ...
  function fchmod(fd, mode, callback) {
    ...
    fd_pool.getfd(fd, function(err, inf) {
      if (err) return callback(err);
      var c = cqlg.change_mode(inf.node.id, mode);
      client.exax(c, make_cass_cb(callback, path, 'fchmod'));
    });
  }
  ...
  function rmdir(path, callback) {
    ...
    pathToNodeid('rmdir', path, 0, function(err, inf) {
      ...
      if (inf.path.type != cqld.T_DIR) {
        return callback(new FileError('is not directories', path, 'EPERM', 'rmdir'));
      }
      if (inf.path.child) {
        return callback(new FileError(
  ...
  function open(path, flags, _mode, callback) {
    ...
    _create_node('open', path, _mode, flag.throw_if_exist, cqld.T_FILE,
      function(err, inf) {
        ...
        if (!inf.path) console.log('!!!open', inf)
        var fdobj = {
          flag     : flag,
          path     : inf.path,
          node     : inf.node,
  ...
  function createReadStream(path, _options) {
    ...
    reader._read = function(size) {
      _open(function() {
        var buf = new Buffer(blksz);
        read(fd, buf, 0, blksz, pos, function(err, rlen, rbuf) {
          if (err) return reader.emit(err);
          if (pos >= tsize) {
            reader.push(null);
          } else if (rlen > 0) {
            pos += rlen;
    ...
    function _close() {
      if (!fd) return;
      fd = null;
      if (_options.autoClose && _options.fd) {
        close(_options.fd, function(err) {
          if (err) reader.emit('error', err);
        });
      }
    }

    function _get_stat(_next) {
      fstat(fd, function(err, _inf) {
        if (tsize <= 0 || tsize < pos || tsize > _inf.size) {
          tsize = _inf.size;
        }
        blksz = _inf.blksize;
        _next();
  ...
  function createWriteStream(path, _options) {
    ...
    writer._write = function(chunk, encoding, callback) {
      _open(function() {
        write(fd, chunk, 0, chunk.length, pos, callback);
        pos += chunk.length;
      });
    };
