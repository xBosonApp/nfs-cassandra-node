...
//
function fs(client, _rediscli, hdid, return_fs_cb) {
  var ret = {
    rename          : rename,
    renameSync      : none,
    ftruncate       : ftruncate,
...
return init();
  ...
  function watch(filename, _options, _listener) {
    ...
    if (_options.recursive) {
      ...
      channel   = notify_perfix + filename + '*';
      tfn       = 'tpsubscribe';
    } else {
    ...
    var channel = _rediscli[tfn](channel, function(err, type, _fname) {
      if (err) {
  ...
  // 发送文件更改消息
  function _change(filename, type) {
    filename = _real_path(filename);
  ...
  function createReadStream(path, _options) {
    ...
    function _close() {
      if (!fd) return;
      if (_options.fd && (!_options.autoClose)) {
        reader.emit('close');
      } else {
