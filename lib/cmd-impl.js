...
cmd.zip = function zip(av, next) {
  ...
  function writezip() {
    zip.toBuffer(function(buf) {
      df.fs.writeFile(df.path, buf, next);
    }, next);
  }
};
...
cmd.uzip = function uzip(av, next) {
  ...
  sf.fs.readFile(sf.path, function(err, buffer) {
    ...
    function eachEnt() {
      if (++i < ens.length) {
        var ent = ens[i];
        var fname = plib.join(df.path, ent.entryName);
        ...
        if (ent.isDirectory) {
          ...
          df.fs.writeFile(fname, ent.getData(), 'wx', function(err) {
            if (err) return next(err);
            log('file', fname);
            eachEnt();
          });
