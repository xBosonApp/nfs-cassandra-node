...
helper.rmdir = 'dirname [-r]';
cmd.rmdir = function rmdir(av, next) {
  var fp = gtfp(needav(av[1]));
  var recursive = (av[2] == '-r');
  if (recursive) {
    var base = plib.dirname(fp.path);
    var name = plib.basename(fp.path);

    walkdir(fp.fs, base, name, function(err, files) {
      ...
      function eachfile() {
        if (++i < files.length) {
          var file = files[i];
          if (file.type == 'file') {
            fp.fs.unlink(file.complete, function(e) {
              if (e) return next(e);
              log('X file ', file.complete);
              eachfile();
            });
          } else {
            fp.fs.rmdir(file.complete, function(e) {
              if (e) return next(e);
              log('X dir  ', file.complete);
              eachfile();
            });
          }
        } else {
